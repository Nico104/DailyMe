...
import 'package:dailyme/features/widgets/future_note_button.dart';
...
class _TodayScreenState extends State<TodayScreen> {
  ...
  Future<Map<String, dynamic>> _loadToday() async {
    ...
    // Always ensure normalized {date, note, rating, future_note}
    final initialData = {
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState != ConnectionState.done) {
          return const Scaffold(body: CustomLoadingIndicatior());
        }
        final todayData = snapshot.data ?? {'note': '', 'mood': null};
        final String? futureNote = todayData['future_note'] is String && (todayData['future_note'] as String).trim().isNotEmpty
            ? todayData['future_note'] as String
            : null;
        return Scaffold(
          appBar: AppBar(
            leadingWidth: 35.w,
            leading: futureNote != null
                ? FutureNoteButton(futureNote: futureNote!)
                : null,
            title: Text(
              'Today',
